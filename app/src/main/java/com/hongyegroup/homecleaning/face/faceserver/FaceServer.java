package com.hongyegroup.homecleaning.face.faceserver;
...
import com.hongyegroup.homecleaning.face.model.FaceRegisterInfo;
...
public class FaceServer {
    ...
    public boolean init(Context context) {
        synchronized (this) {
            if (faceEngine == null && context != null) {
                ...
                int engineCode = faceEngine.init(context, DetectMode.ASF_DETECT_MODE_IMAGE,
                        DetectFaceOrientPriority.ASF_OP_0_ONLY, 16, 1,
        ...
        synchronized (this) {
            ...
            for (File featureFile : featureFiles) {
                try {
                    ...
                    faceRegisterInfoList.add(new FaceRegisterInfo(feature, userId, userName));
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
        ...
        synchronized (this) {
            ...
            if (code != ErrorInfo.MOK) {
                ...
                try {
                    ...
                    faceRegisterInfoList.add(new FaceRegisterInfo(faceFeature.getFeatureData(), "0", userName));
                    return true;
                } catch (IOException e) {
                    e.printStackTrace();
                    return false;
            ...
            if (code == ErrorInfo.MOK && faceInfoList.size() > 0) {
                ...
                try {
                    ...
                    if (code == ErrorInfo.MOK) {
                        ...
                        faceRegisterInfoList.add(new FaceRegisterInfo(faceFeature.getFeatureData(), userId, userName1));
                        return true;
                    } else {
    ...
    public CompareResult getTopOfFaceLib(FaceFeature faceFeature) {
        ...
        for (int i = 0; i < faceRegisterInfoList.size(); i++) {
            tempFaceFeature.setFeatureData(faceRegisterInfoList.get(i).featureData);
        ...
        if (maxSimilarIndex != -1) {
            return new CompareResult(faceRegisterInfoList.get(maxSimilarIndex).userId, faceRegisterInfoList.get(maxSimilarIndex).userName, maxSimilar);
        }
        return null;
    }
