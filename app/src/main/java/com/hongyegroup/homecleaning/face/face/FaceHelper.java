...
import com.hongyegroup.homecleaning.face.utils.TrackUtil;
    ...
    public void requestFaceFeature(byte[] nv21, FaceInfo faceInfo, int width, int height, int format, Integer trackId) {
        if (faceListener != null) {
            if (frEngine != null && frThreadQueue.remainingCapacity() > 0) {
                ...
                faceListener.onFaceFeatureInfoGet(null, trackId, ERROR_BUSY, nv21, faceInfo, width, height);
            }
        }
    }
...
//                        Log.i(TAG, "run: fr costTime = " + (System.currentTimeMillis() - frStartTime) + "ms");
                        faceListener.onFaceFeatureInfoGet(faceFeature, trackId, frCode, nv21Data, faceInfo, width, height);
                    } else {
                        faceListener.onFaceFeatureInfoGet(null, trackId, frCode, nv21Data, faceInfo, width, height);
                        faceListener.onFail(new Exception("fr failed errorCode is " + frCode));
                    }
                } else {
                    faceListener.onFaceFeatureInfoGet(null, trackId, ERROR_FR_ENGINE_IS_NULL, nv21Data, faceInfo, width, height);
